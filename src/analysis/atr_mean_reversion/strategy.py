...
def detect_atr_mean_reversion_signals(
    ...
    Args:
        ...
        require_adx: Whether to require ADX filter (default: False)
...
def calculate_atr_based_outcome(
    ...
    """
    # Need at least one more day after signal for entry
    if entry_idx + 1 + period >= len(df):
        return None

    # CRITICAL: Enter at OPEN of next day to avoid look-ahead bias
    # Signal detected at close of entry_idx, enter next day
    entry_price = df['Open'].iloc[entry_idx + 1]
    entry_atr = df['ATR'].iloc[entry_idx]

    if pd.isna(entry_atr) or entry_atr <= 0 or pd.isna(entry_price):
        return None
    ...
    # Forward window starts from entry day (entry_idx + 1), not signal day
    forward_window = df.iloc[entry_idx + 1:entry_idx + 1 + period]
...
def analyze_forward_days(
    ...
    Note: entry_idx is the SIGNAL day. Entry happens next day.
    """
    if entry_idx + 1 + period >= len(df):
        return None

    # Signal detected at close of entry_idx
    signal_date = df.index[entry_idx]
    # Actual entry at open of next day
    entry_date = df.index[entry_idx + 1]
    entry_price = df['Open'].iloc[entry_idx + 1]

    # Signal characteristics (from signal day)
    ...
    # Forward window for verification (from entry day)
    forward_window = df.iloc[entry_idx + 1:entry_idx + 1 + period]
...
def run_analysis(
    ...
    # Drop NaN (include ADX if filtering is enabled)
    dropna_cols = ['SMA200', 'RSI', 'ATR']
    if require_adx and 'ADX' in df.columns:
        dropna_cols.append('ADX')
...
def main():
    ...
    parser.add_argument('--enable-adx', action='store_true', help='Enable ADX filter (disabled by default)')
            ...
            if not results_df.empty:
                ...
                if args.sp500:
                    ...
                    print_summary(
                        ...
                        show_signal_dates=not args.show_signal_details,  # Show simple dates if not showing details
                        ...
                        settings={
                            ...
                            'ADX threshold': args.adx_threshold if args.enable_adx else 'disabled',
